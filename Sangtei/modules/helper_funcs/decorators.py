...
from Sangtei import dispatcher as d, log
from typing import Optional, Union, List


class SangteiTelegramHandler:
    def __init__(self, d):
        self._dispatcher = d
    ...
    def command(
        ...
        def _command(func):
            try:
                ...
                log.debug(f"[KYOCMD] Loaded handler {command} for function {func.__name__} in group {group}")
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                        DisableAbleCommandHandler(command, func, filters=filters, run_async=run_async,
                ...
                log.debug(f"[KYOCMD] Loaded handler {command} for function {func.__name__}")
    ...
    def message(self, pattern: Optional[BaseFilter] = None, can_disable: bool = True, run_async: bool = True,
        ...
        def _message(func):
            try:
                ...
                log.debug(f"[KYOMSG] Loaded filter pattern {pattern} for function {func.__name__} in group {group}")
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                log.debug(f"[KYOMSG] Loaded filter pattern {pattern} for function {func.__name__}")
    ...
    def callbackquery(self, pattern: str = None, run_async: bool = True):
        def _callbackquery(func):
            ...
            log.debug(f'[KYOCALLBACK] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}')
            return func
...
rencmd = SangteiTelegramHandler(d).command
renmsg = SangteiTelegramHandler(d).message
rencallback = SangteiTelegramHandler(d).callbackquery
reninline = SangteiTelegramHandler(d).inlinequery
