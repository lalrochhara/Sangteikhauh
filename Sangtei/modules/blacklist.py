...
from Sangtei import log, dispatcher
from Sangtei.modules.helper_funcs.chat_status import user_admin as u_admin, user_not_admin
...
from Sangtei.modules.warn import warn
...
def kontol_help(update: Update, context: CallbackContext):
    ...
    bot = context.bot
    help_info = query.data.split("kontol_help_")[1]
    if help_info == "blacklist":
        help_text = gs(update.effective_chat.id, "blacklist_help")
    elif help_info == "tikelblk":
        help_text = gs(update.effective_chat.id, "tikelblk_help")
