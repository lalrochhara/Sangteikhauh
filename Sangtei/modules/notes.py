import re, ast, random, html
from io import BytesIO
from typing import Optional

import Sangtei.modules.sql.notes_sql as sql
from Sangtei import log, dispatcher, SUDO_USERS
from Sangtei.modules.helper_funcs.chat_status import connection_status
...
from Sangtei.modules.helper_funcs.decorators import rencmd, renmsg, rencallback
...
@rencmd(command="get", pass_args=True)
@typing_action
@connection_status
...
@rencmd(command=["removeallnotes", "clearall"], pass_args=True)
@typing_action
...
@rencmd(command=["notes", "saved"], pass_args=True)
@connection_status
def list_notes(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    note_list = sql.get_all_chat_notes(chat_id)
    notes = len(note_list) + 1
    msg = "*List of notes in Log:* \nYou can retrieve these notes by using `/get notename` or `#notename`\n\n"
