...
def get_user_list(key):
    ...
    from Sangtei.modules.sql import nation_sql
    royals = nation_sql.get_royals(key)
    return [a.user_id for a in royals]

FORMAT = "[Sangtei] %(message)s"
logging.basicConfig(
...
LOGGER = logging.getLogger('[Sangtei]')
LOGGER.info("Sangtei is starting. | Rendy Projects. | Licensed under GPLv3.")
...
#print(flag)
log = logging.getLogger('[Sangtei]')
logging.getLogger('ptbcontrib.postgres_persistence.postgrespersistence').setLevel(logging.WARNING)
logging.getLogger("pyrogram").setLevel(logging.INFO)
log.info("[Sangtei] Takehito is starting. | Rendy Project. | Licensed under GPLv3.")
log.info("[Sangtei] Not affiliated to other anime.")
log.info("[Sangtei] Project maintained by: github.com/Randi356 (t.me/rencprx)")

# if version < 3.6, stop bot.
if sys.version_info[0] < 3 or sys.version_info[1] < 7:
    log.error(
        "[Sangtei] You MUST have a python version of at least 3.7! Multiple features depend on this. Bot quitting."
    )
    quit(1)
...
class SangteiINIT:
    def __init__(self, parser: ConfigParser):
        ...
        self.MONGO_DB: str = self.parser.get('MONGO_DB', "sangteiubot")
...
KInit = SangteiINIT(parser=renconfig)
...
try:
    REDIS.ping()
    LOGGER.info("[Sangtei] Your redis server is now alive!")
...
from Sangtei.modules.helper_funcs.handlers import CustomCommandHandler
